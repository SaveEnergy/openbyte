...
- `sync.Once` on close/stop paths for idempotent shutdown (`Manager`, stream server).
...
- Speed test: **`speedtest-orchestrator.js`** (lifecycle + share) + thin **`openbyte.js`** init; **`speedtest-http.js`** barrels **`speedtest-http-{shared,download,upload}.js`** (shared warmup/progress via **`applyHttpMeasureTick`** in **`speedtest-http-shared.js`**); download **`download-{platform,github}.js`** + **`download.js`**; network **`network-{helpers,health,probes}.js`** + **`network.js`**. Any new top-level **`web/*.js`** (or HTML/CSS) must be added to **`internal/api/router_static.go`** allowlist or the server returns **404**.
...
- **Perf autoresearch**: Tracked Cursor playbook **`test/perf/AUTORESEARCH_CURSOR_COMMAND.md`** (install into **`.cursor/commands/autoresearch.md`**); full rules **`test/perf/PROMPT_AUTORESEARCH.md`**. **`make autoresearch-preflight`** — exit code + **`AUTORESEARCH_*`** lines (next **`N`**, branch name, benchstat command) before creating **`autoresearch/perf-N`**. After a perf branch merges to **`main`**, bump **`autoresearch_counter.txt`**, delete **`autoresearch/perf-N`**, and open the next branch manually (see **`PROMPT_AUTORESEARCH.md`** § After a perf branch merges).
...
| `AZ0Il3awQcX3ClQ7_cdL` | javascript:S7763 | MINOR | `web/network.js` | **MCP 2026-03-20**: stale old barrel export — **verify CLOSED** on next analysis |
...
- **2026-03-20 API handler file split**: **`handlers_response_host.go`** — `normalizeHost` + response host wiring; JSON/metrics/response helpers stay in **`handlers_http.go`**; stream snapshot DTOs + **`toStreamSnapshotResponse`** in **`handler_stream_dto.go`**, route handlers in **`handlers_stream.go`**. Checks: `go test ./internal/api/... ./test/unit/api/... -run TestOpenAPIRouteContract`.
...
- **2026-03-25**: **Sonar OPEN ×2** (MCP) — stale keys on old handler/network layout. Earlier same day: **`20260323-refactor-01`..`04`** test splits.
- **2026-03-24**: **`20260322-refactor-01` Done** — router tests split (CORS/stream ID / static+CSP+rate-limit / results+404 / allowlist+fonts+smoke); same `package api_test`, shared constants in `router_test_common_test.go`.
...
- **2026-03-22**: Deep LOC scan → `20260322-refactor-01`..`09`; tests first, then client/e2e/web/tools.
...
- **2026-03-20**: Local/dev UI — **`router_static`** allowlist + test for **`speedtest-http-{download,shared,upload}.js`** (barrel imports).
