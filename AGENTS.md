...
- **Perf autoresearch**: Tracked Cursor playbook **`test/perf/AUTORESEARCH_CURSOR_COMMAND.md`** (install into **`.cursor/commands/autoresearch.md`**); full rules **`test/perf/PROMPT_AUTORESEARCH.md`**. **`make autoresearch-preflight`** — exit code + **`AUTORESEARCH_*`** lines (next **`N`**, branch name, benchstat command) before creating **`autoresearch/perf-N`**. **`/autoresearch --loop`**: outer iteration via **`make autoresearch-loop-complete`** (merge **`perf-N`** into **`main`**, bump counter, new local **`perf-(N+1)`**); then push **`main`**, delete remote **`perf-N`** if present, baseline scan on the new branch.
