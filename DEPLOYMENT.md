...
- The bundled Traefik HTTPS routers default to `TRAEFIK_TLS_OPTIONS=openbyte-h1@file`, which advertises HTTP/1.1 only for the browser data path. Set `TRAEFIK_TLS_OPTIONS=openbyte-h2@file` only when you intentionally want to compare Traefik HTTP/2.
