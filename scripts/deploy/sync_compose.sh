...
ssh -i "$SSH_TMP_DIR/id_rsa" -o UserKnownHostsFile="$SSH_TMP_DIR/known_hosts" -p "${SSH_PORT:-22}" "${SSH_USER}@${SSH_HOST}" \
  "mkdir -p \"${REMOTE_DIR}/docker\" && if test -d \"${REMOTE_DIR}/docker/traefik-openbyte.yaml\"; then rmdir \"${REMOTE_DIR}/docker/traefik-openbyte.yaml\"; fi"
...
ssh -i "$SSH_TMP_DIR/id_rsa" -o UserKnownHostsFile="$SSH_TMP_DIR/known_hosts" -p "${SSH_PORT:-22}" "${SSH_USER}@${SSH_HOST}" \
  "test -f \"${REMOTE_DIR}/docker/docker-compose.ghcr.yaml\" && test -f \"${REMOTE_DIR}/docker/docker-compose.ghcr.traefik.yaml\" && test -f \"${REMOTE_DIR}/docker/traefik-openbyte.yaml\""
...
local_traefik_config_sha="$(sha256sum docker/traefik-openbyte.yaml | cut -d ' ' -f1)"
...
remote_traefik_config_sha="$(ssh -i "$SSH_TMP_DIR/id_rsa" -o UserKnownHostsFile="$SSH_TMP_DIR/known_hosts" -p "${SSH_PORT:-22}" "${SSH_USER}@${SSH_HOST}" "if command -v sha256sum >/dev/null 2>&1; then sha256sum \"${REMOTE_DIR}/docker/traefik-openbyte.yaml\" | cut -d ' ' -f1; else shasum -a 256 \"${REMOTE_DIR}/docker/traefik-openbyte.yaml\" | cut -d ' ' -f1; fi")"
...
if [[ "$local_traefik_config_sha" != "$remote_traefik_config_sha" ]]; then
  echo "checksum mismatch for traefik-openbyte.yaml"
  exit 1
fi
